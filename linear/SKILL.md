...
3. **Unmapped Repository Self-Bootstrapping**: If no mapping exists for the current repository, follow [references/bootstrapping.md](references/bootstrapping.md) to query `organization { name urlKey }` and `teams { nodes { id name key } }`. Never guess or auto-select a team based on repo name or single-team result; require explicit user selection or an authoritative team URL/key before writing `<repo>/.linear/team.json` and updating `~/.agents/linear/profiles.json`.
